...
export default {
  ...
  methods: {
    ...
    drag (event: Event, offset: { left: number, top: number }) {
      ...
      this.actualValue = this.round(this.valueFromBounds(offset.left, offsetWidth,offset.top))
    ...
    valueFromBounds (point: number, width: number, top: number) {
      if(this.ori === true){     
        return ((width - top) / width) * (this._max - this._min) + this._min
      }else{
